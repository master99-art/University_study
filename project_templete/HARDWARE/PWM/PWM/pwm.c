...
void PWM_Init(void)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM5, ENABLE);			//开启TIM2的时钟
	...
	TIM_OC1Init(TIM5, &TIM_OCInitStructure);                        //将结构体变量交给TIM_OC3Init，配置TIM2的输出比较通道3

	TIM_OCInitTypeDef TIM_OCInitStructure0;							//定义结构体变量
                                                                 //则最好执行此函数，给结构体所有成员都赋一个默认值
	                                                                //避免结构体初值不确定的问题
	TIM_OCInitStructure0.TIM_OCMode = TIM_OCMode_PWM1;               //输出比较模式，选择PWM模式1
	TIM_OCInitStructure0.TIM_OCPolarity = TIM_OCPolarity_High;       //输出极性，选择为高，若选择极性为低，则输出高低电平取反
	TIM_OCInitStructure0.TIM_OutputState = TIM_OutputState_Enable;   //输出使能
	TIM_OCInitStructure0.TIM_Pulse = 0;								//初始的CCR值
	TIM_OC2Init(TIM5, &TIM_OCInitStructure0);                        //将结构体变量交给TIM_OC3Init，配置TIM2的输出比较通道3
...
///////////////////TIM4
	...
	GPIO_Init(GPIOD, &GPIO_InitStructure1);							//将PA2引脚初始化为复用推挽输出	
																	//受外设控制的引脚，均需要配置为复用模式
	...
	TIM_TimeBaseInitStructure1.TIM_Period = 20000 - 1;                 //计数周期，即ARR的值
	TIM_TimeBaseInitStructure1.TIM_Prescaler = 72 - 1;               //预分频器，即PSC的值
	...
	TIM_OC1Init(TIM4, &TIM_OCInitStructure1);                        //将结构体变量交给TIM_OC3Init，配置TIM2的输出比较通道3
...
{
	TIM_SetCompare1(TIM4,(uint16_t)(Angle / 180 * 2000 + 500));	//设置占空比
